...
```{r ans21}
commitCheck[[8]][["commit"]][["committer"]][["date"]] -> a
library(lubridate)
ymd_hms(a) -> commit8Date
commit8Date
```
...
```{r ans22}
with_tz(commit8Date , tzone = "Asia/Taipei") -> commit8TWDate
commit8TWDate
```
...
length(commitCheck)
library(stringr)
str_c("commit",c(1:30)) -> names(commitCheck)
names(commitCheck)
```
...
```{r ans25}
ymd_hms(allCommitDates) -> b
with_tz(b , tzone = "Asia/Taipei") -> allCommitDatesTW
allCommitDatesTW
```
...
```{r ans26}
hour(allCommitDatesTW) -> c
minute(allCommitDatesTW) -> d
(c == 4)&(d >= 30) -> e
c == 5 -> f
(c == 6)&(d <= 30) -> g
e|f|g -> logicWithinTimeFrame
...
```{r ans34}
transcriptPartialData[[1]][c(logicFromEcon)] -> h
transcriptPartialData[[2]][c(logicFromEcon)] -> i
transcriptPartialData[[3]][c(logicFromEcon)] -> j
names(h) -> "學號"
names(i) -> "學分數"
names(j) -> "必選修類別（必∕選∕通）"
list(h,i,j) -> transcriptPartialDataEcon
